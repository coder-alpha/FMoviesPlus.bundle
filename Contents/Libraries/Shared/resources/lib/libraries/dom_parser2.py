...
def parse_dom(html, name='', attrs=None, req=False, exclude_comments=False):
    ...
    name = name.strip()
    # print ('parse_dom: Name: |%s| Attrs: |%s| Ret: |%s| - HTML: %s' % (name, attrs, req, type(html)), log_utils.LOGDEBUG)
    if isinstance(html, unicode) or isinstance(html, DomMatch):
        ...
        try:
            ...
            print ("HTML Decode Failed. Data length: %d" % len(html), log_utils.LOGWARNING)
            try:
                html = [html.decode("utf-8", "replace")]
            except:
                print ("HTML Decode Failed (Replace). Data length: %d" % len(html), log_utils.LOGWARNING)
                html = [html]
    elif not isinstance(html, list):
        print ("Input isn't list or string/unicode.", log_utils.LOGWARNING)
        return ''

    if not name:
        print ("Missing tag name", log_utils.LOGWARNING)
        return ''
    
    if not isinstance(attrs, dict):
        print ("Attrs must be dictionary", log_utils.LOGWARNING)
        return ''
    ...
    # print ("Done: %s" + (all_results), xbmc.LOGDEBUG)
    return all_results
